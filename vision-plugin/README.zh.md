...
```sh
# 测试运行在本包自己的 node_modules（npm 依赖）上
pnpm install
vitest run
# 类型检查：单一自包含 tsconfig.json 同时覆盖 host 程序（src/）与浏览器端（src/client）
tsc --noEmit
# 双面构建：lib/index.js（host）+ lib/client.js（浏览器 lazy-CJS bundle）
tsdown
```

所有 `@deepseek-ai/*` 包、react 与工具链（typescript、vitest、tsdown、lightningcss）都通过 npm 从本包自己的 `node_modules` 解析（`^0.1.0-rc.6`）。`tsdown.config.ts` 是**本地 standalone 双面构建**（host `lib/index.js` + 浏览器 `lib/client.js`），从 harness 的 `clientBundle` 预设 vendor 而来（同 dsh-visual-plugin 做法）——**开发、测试、构建都不再需要 harness 检出**。浏览器端是 `src/client/` 下的 TypeScript + React，编译为 `lib/client.js`，CSS Modules 在物化时以 `<style data-plugin>` 标签注入。
